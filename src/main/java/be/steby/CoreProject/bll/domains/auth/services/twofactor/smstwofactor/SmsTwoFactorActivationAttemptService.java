...
/**
 * Service for rate limiting SMS 2FA activation attempts.
 *
 * Prevents abuse of SMS sending by limiting how many activation codes
 * can be requested within a time window.
 *
 * Configuration:
...
@Service
@Slf4j
public class SmsTwoFactorActivationAttemptService {
    ...
    /**
     * Check if user has exceeded activation attempt limit.
     *
    ...
    public boolean hasExceededAttempts(User user) {
        ...
        if (exceeded) {
            log.warn("User {} exceeded SMS 2FA activation attempts ({}/{})",
    ...
    /**
     * Record an activation attempt for a user.
     *
    ...
    public void recordAttempt(User user) {
        ...
        log.debug("Recorded SMS 2FA activation attempt for user: {}", user.getUsername());
    }

    /**
     * Clear attempts for a user (called on successful activation).
     *
     * @param user The user whose attempts to clear
     */
    public void clearAttempts(User user) {
        attemptCache.remove(getKey(user));
        log.debug("Cleared SMS 2FA activation attempts for user: {}", user.getUsername());
    }

    private String getKey(User user) {
        return "sms_activation:" + user.getPublicId();
    }
