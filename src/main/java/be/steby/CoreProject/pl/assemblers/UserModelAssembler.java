...
import be.steby.CoreProject.pl.controllers.AdminController;
...
public class UserModelAssembler implements RepresentationModelAssembler<User, EntityModel<UserDTO>> {
    ...
    public EntityModel<UserDTO> toModel(User user) {
        ...
        return EntityModel.of(userDTO,
                linkTo(methodOn(AdminController.class).getUserById(userDTO.id())).withSelfRel());
    }
    ...
    public EntityModel<UserDTO> toModelWithPermissions(User user, User authenticatedUser, boolean isSuperAdmin) {
        ...
        model.add(linkTo(methodOn(AdminController.class).getUserById(userDTO.id())).withSelfRel());
        ...
        if (!userDTO.id().equals(authenticatedUser.getId())) {
            model.add(linkTo(methodOn(AdminController.class).activateUser(userDTO.id())).withRel("activate"));
            model.add(linkTo(methodOn(AdminController.class).deactivateUser(userDTO.id())).withRel("deactivate"));
            ...
            if (!isUserSuperAdmin || isSuperAdmin) {
                model.add(linkTo(methodOn(AdminController.class).grantUserRole(userDTO.id(), null))
                        .withRel("grant-role"));
                model.add(linkTo(methodOn(AdminController.class).revokeUserRole(userDTO.id(), null))
                        .withRel("revoke-role"));
                model.add(linkTo(methodOn(AdminController.class).deleteUser(userDTO.id()))
                        .withRel("delete"));
            }
        }
